...
import debugpy 
from armonik.common.objects import TaskOptions
...
def vec_add(ctx, a: np.ndarray, b: np.ndarray, **kwargs) -> np.ndarray:
    """
    ...
    partition_name = ctx.task_handler.task_options.partition_id
    ctx.logger.info(f"Executing vec add in partition {partition_name}")
    ...
    if a.size > VECTOR_SIZE_THRESHOLD:
        ...
        result_handle1 = vec_add.invoke(a1, b1, pmk_partition_id="pymonik2")
        result_handle2 = vec_add.invoke(a2, b2, pmk_partition_id="pymonik2")
...
if __name__ == "__main__":
    ...
    with Pymonik(endpoint="localhost:5001", partition=["pymonik", "pymonik2"], environment={"pip": ["numpy"]}) as pymonik:
        ...
        final_result_handle = vec_add.invoke(vec_a, vec_b, task_options= TaskOptions(partition_id="pymonik2", max_duration=timedelta(seconds=12), priority=1, max_retries=2))
